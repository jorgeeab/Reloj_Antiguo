...
class RelojSimHierbaEnv(gym.GoalEnv):
    ...
    def __init__(self):
     ...
     #   print("Arco MAXImo", self.ArcoMaximo)
       ...
       # self.AumentoCamara = 0.3  # cm/cm   ## la relacion entre la  camara(cm) y la altura o "POSY" (cm)
        ...
        self.PosAreal = 0  # angulo
        self.PosXreal = self.MinX  # cm
        self.PosYreal = 0  # cm/

        ## Para conocer la desviacion virtual
        self.DesvMaxA = 6  #en grados
        self.DesvA = 0
        self.DesvX = 0
        self.DesvZ = 0
    ...
    def ImportarMapa(self,Plantas,ImagenesPlantas,CarpetaMapa):
        ...
        try:
           ...
           # print("PlantasVirtuales =", self.PlantasVirtuales)
    ...
    def GenerarMapaVirtual(self,DibujarMarcas=False):
        ...
        # try:
          #  print("Generando Terreno Virtual")
            ...
            alto1, ancho1 = self.fondo.shape[:2]
...
#
            ...
            if DibujarMarcas:
                fondo = self.Mappeador.DibujarMarcas(fondo)
    ...
    def SimularMovimiento(self,EMX, EMA ,EMY, EBE): ## para simular un movimiento

        try:
            #Realizar La simulacion de los movimientos hechos por el robot ...
            ...
            for movimiento in self.ListaMovimientos:
                 ...
                 if diferenciaenms > self.Atrazo:
                    ...
                    if  -0.4 < EMA < 0.4: # DesAcelerar
                       self.VelocidadA=0
                    NuevaPosA =  self.PosAR + self.VelocidadA * self.TiempoPaso ## la velocidad por lo que duro el paso anterior para deteminar cuanto se movio
                    if NuevaPosA <= self.MaxA and NuevaPosA >=0:
                        self.PosAR =  NuevaPosA
                    ...
                    NuevaPosX =  self.PosXR + self.VelocidadX * self.TiempoPaso ## la velocidad por lo que duro el paso anterior para deteminar cuanto se movio
                    if NuevaPosX <= self.MaxX and NuevaPosX >=self.MinX:
                        self.PosXR =  NuevaPosX



                    ## Agregar desviacio random al eje A

                    self.DesvA = self.DesvMaxA * np.random.random()

                    self.PosXreal= self.PosXR
                    self.PosXR = self.PosXreal+self.DesvA
                    ...
                    if -0.4 < EMY < 0.4:  # DesAcelerar
                      self.VelocidadY = 0
                    NuevaPosY =  self.PosYR + self.VelocidadY* self.TiempoPaso ## la velocidad por lo que duro el paso anterior para deteminar cuanto se movio
                    if NuevaPosY <= self.MaxY and NuevaPosY >= 0 :
                        self.PosYR =  NuevaPosY
                    self.PosYreal = self.PosYR
    ...
    def SimularCamara(self):
        ...
        try:
            ...
            if self.PosAreal >= 0 and self.PosAreal < 90:
                angulo = self.PosAreal - 90

            if self.PosAreal >= 90 and self.PosAreal < 180:
                angulo = self.PosA + 270


            if self.PosAreal >= 180 and self.PosA < 270:
                angulo = self.PosAreal - 90


            if self.PosA >= 270 and self.PosA < 860:
                angulo = self.PosAreal +270
 ...
 #               print("Velocidad AMayor", self.VelocidadAMaxima)
  ...
  #              print("Velocidad XMayor", self.VelocidadXMaxima)
        ...
        #
    def ProcesarImagen2(self,camara):

        if self.VelocidadA < self.VelocidadAMaxima * 0.2 or self.VelocidadA > self.VelocidadAMinima * 0.2:

            self.PosAAnterior = self.PosAR  # angulo
            self.PosXAnterior = self.PosXR  # cm
            self.PosYAnterior = self.PosYR  # cm
    ...
    def Regar (self,VOL):
       ...
       if self.ModoVirtual == False:
           ...
           while self.VOLDetectado > 0:
               ...
               self.EjecutarAccion(EMA=0, EBE=0, EMX=0, EMY=0, VOL=0, Reset=0)
               self.RecibirDatos()
               time.sleep(0.1)
           print("listo")
