...
class RelojSimHierbaEnv(gym.GoalEnv):
    ...
    def __init__(self):
     ...
     #   print("Arco MAXImo", self.ArcoMaximo)
       ...
       # print( range(self.DistanciaPlantas, self.MaxX*2 - self.DistanciaPlantas,
        ...
        self.AtrazoRandom = True
        self.AtrazoMinimo= 40
        self.AtrazoMaximo = 1000  # milisegundos
        ...
        self.AceleracionAMinima=0.1*self.VelocidadAMaxima
        self.AceleracionAMaxima=0.5*self.VelocidadAMaxima

        self.AceleracionX =0.3*self.VelocidadXMaxima
        self.AceleracionXMinima = 0.1 * self.VelocidadXMaxima
        self.AceleracionXMaxima = 0.5 * self.VelocidadXMaxima

        self.AceleracionY =0.3*self.VelocidadYMaxima
        self.AceleracionYMinima = 0.1 * self.VelocidadYMaxima
        self.AceleracionYMaxima = 0.5 * self.VelocidadYMaxima


        self.AceleracionRandom = True


        self.TiempoVelocidadAnterior = self.TiempoActual
        self.TiempoVelocidad = 500  # milisegundos
        ...
        self.fondo = cv2.imread("C:/Users/Jorge/PycharmProjects/ProyectoReloj/Imagenes/tierra/fondos/fondotierra.jpg", cv2.IMREAD_COLOR)
        print(os.listdir("Imagenes/tierra/fondos"))
        ...
        print(self.AnchoMapa,self.AltoMapa)
        self.fondo = cv2.resize(src=self.fondo,dsize=(self.AnchoMapa, self.AltoMapa))
   ...
   #     fondo = cv2.imread("Imagenes/tierra/fondotierra.jpg", cv2.IMREAD_COLOR)
      #  fondo = cv2.resize(fondo, (self.AnchoMapa, self.AltoMapa))
        ...
        ## coloca una pausa en cada paso
        self.Manual = False
        self.DiagnosticarParametros = False
        self.AceleracionRandom=False
        self.AtrazoRandom=False
    ...
    def EjecutarAccion(self, EMX,EMY, EBE, EMA,VOL, Reset):
           ...
           try :
               if self.wifi:
                        ...
                        textoAenviar = "{},{},{}".format(round(EMA, 3), round(EMX, 3),round(VOL, 3))  ## al poner una coma al frente indica que es un valor
...
#                        print("texto enviar : ", textoAenviar)
    ...
    def SimularMovimiento(self,EMX, EMA ,EMY, EBE):
        ...
        try:
            ...
            if self.AtrazoRandom: ### atrazo minimo  300   maximo 1000
                nuevoAtrazo = self.Atrazo + np.random.randint(low=-20,high=30)
                if nuevoAtrazo >= self.AtrazoMinimo and  nuevoAtrazo <= self.AtrazoMaximo:
                    self.Atrazo = nuevoAtrazo


            if self.AceleracionRandom :

                nuevaAceleracionA = self.AceleracionA +  (np.random.randint(low=-20, high=30)/100)*self.AceleracionAMaxima
                if nuevaAceleracionA >= self.AceleracionAMinima and nuevaAceleracionA <= self.AceleracionAMaxima :
                    self.AceleracionA = nuevaAceleracionA

                nuevaAceleracionX = self.AceleracionX + (
                            np.random.randint(low=-20, high=30) / 100) * self.AceleracionXMaxima
                if nuevaAceleracionX >= self.AceleracionXMinima and nuevaAceleracionX <= self.AceleracionXMaxima:
                    self.AceleracionX = nuevaAceleracionX

                nuevaAceleracionY = self.AceleracionY + (
                            np.random.randint(low=-20, high=30) / 100) * self.AceleracionYMaxima
                if nuevaAceleracionY >= self.AceleracionYMinima and nuevaAceleracionY <= self.AceleracionYMaxima:
                    self.AceleracionY = nuevaAceleracionY
    ...
    def ProcesarSensores(self):
        ...
        if self.ModoVirtual:
            ...
            self.PosXDetectado, self.PosYDetectado, self.PosZDetectado = self.RelojACuadro(self.PosAR, self.PosXR,
                                                                                           self.PosYR, self.MaxX,
                                                                                           self.MaxX)
