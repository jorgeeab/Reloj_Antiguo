...
class RelojSimHierbaEnv(gym.GoalEnv):
    ...
    def __init__(self):
       ...
       # self.AumentoCamara = 0.3  # cm/cm   ## la relacion entre la  camara(cm) y la altura o "POSY" (cm)
        ...
        self.Plantas = []
        self.CantidadPlantas = 10  # el numero de plantas generadas random
        self.ContadorPlantas = 0
        self.DistanciaPlantas = 20
       ...
       # print( range(self.DistanciaPlantas, self.MaxX*2 - self.DistanciaPlantas,
        ...
        for xplanta in range(self.DistanciaPlantas, self.MaxX*2 - self.DistanciaPlantas,
                             self.DistanciaPlantas):  # (cm)
            for yplanta in range(self.DistanciaPlantas, self.MaxX*2 - self.DistanciaPlantas, self.DistanciaPlantas):
        ...
        self.Atrazo = 1000
        self.AtrazoMaximo = 3000  # milisegundos

        self.VelocidadA = 0.03  ## grados /milisegundos
        self.VelocidadAMaxima = 0.05
        self.VelocidadAMinima = 0.01

        self.VelocidadX = 0.009## cm/ milisegundos .. ejecucion cada intervalo de comunicacion
        self.VelocidadXMaxima = 0.01
        self.VelocidadXMinima = 0.003


        ## suponiendo que hacen falta centimetros
        self.VelocidadY = 0.003
        self.VelocidadYMaxima = 0.005
        self.VelocidadYMimima = 0.003
        ...
        self.fondo = cv2.imread("Imagenes/tierra/fondotierra.jpg", cv2.IMREAD_COLOR)
        ...
        #Aruco Marks
        self.DistanciaMarcas= 10 #(cm)
        self.LadoMarcas = 2.5#(cm)
        ...
        self.NombresSensores=[
                             "XRequerido", "YRequerido", "ZRequerido",
                             "PosXDetectado", "PosYDetectado", "PosZDetectado",
                             "Error",
                             # "F1x", "F1y", "F1a","F1d",
                             # "F2x", "F2y", "F2a","F2d",
                             # "FMx", "FMy","FMd",
                             ...
                             "XMinima", "XMaxima","YMinima", "YMaxima", ##Los cuatro puntos delimitadores de la zona  cuadrada en la que se esta trabajando
                            # "PorcentajeMapa", "PorcentajePlantas","PorcentajeHierbas",   ## punto de mapeo más cercano
                             "EMXDetectado", "EMYDetectado", "EMADetectado"
                              ]

        self.Observaciones = np.array(
            [self.XRequerido ,self.YRequerido,self.ZRequerido ,
             self.PosXDetectado, self.PosYDetectado, self.PosZDetectado,
             self.Error,
        ...
        self.ObservacionesMaximas = np.array(
            [self.MaxX*2,self.MaxX*2,self.MaxY,
             self.MaxX*2,self.MaxX*2, self.MaxY,
             self.ErrorMaximo,
        ...
        self.ObjetivoMaximo = np.array(
            [self.MaxX * 2, self.MaxX * 2, self.MaxY, self.ErrorMaximo])
        ...
        self.ObjetivoDeseado = np.array(
            [self.XRequerido, self.YRequerido, self.ZRequerido, self.E])
        self.ObjetivoDeseado = np.divide(self.ObjetivoDeseado, self.ObjetivoMaximo)

        self.PesosoObjetivos = np.array([0.4, 0.4, 0.0, 0.2])  ## Para Moverse a un punto especifico
        ...
        n_Observaciones = len(self.Observaciones)
        ...
        if self.ModoVirtual == False:
            ...
            #self.ConexionRobot = Conexion(wifi=self.wifi,puerto=0,baudios=115200)
    ...
    def EjecutarAccion(self, EMX,EMY, EBE, EMA, Reset):

       # Enviar Accioon
        if self.wifi:

            textoAenviar = "{},{},".format(round(EMA, 3), round(EMX, 3))  ## al poner una coma al frente indica que es un valor
          ...
          #  print("texto enviar : ", textoAenviar)
           ...
           # print("entrada : ", entrada)
            ...
            self.Data = json.loads(self.arduinoString)


            self.arduinoString = ""
            self.Sensores = pd.DataFrame(self.Data['Sensores'])  # las variables que no controlo
            self.Posicion = (
            self.Data['Sensores']['Angulo'][0], self.Data['Sensores']['PosX'][0], self.Data['Sensores']['PosY'][0])
            ac = self.Data['Actuadores']
        ...
        try:
            ...
            for movimiento in self.ListaMovimientos:
                 ...
                 if diferenciaenms > self.Atrazo:
                    ...
                    if EMA > 0.5 and self.PosA <= self.MaxA:
                        self.PosA += self.VelocidadA * self.TiempoPaso ## la velocidad por lo que duro el paso anterior para deteminar cuanto se movio

                    if EMA < -0.5 and self.PosA >= 0:
                        self.PosA -= self.VelocidadA* self.TiempoPaso
                    # moviendo X

                    if EMX > 0.5 and self.PosX <= self.MaxX:
                        self.PosX += self.VelocidadX * EMX* self.TiempoPaso
                        ## el valor negativo lo da EMX
                    if EMX < -0.5 and self.PosX >= self.MinX:
                        self.PosX += self.VelocidadX * EMX* self.TiempoPaso

                    # moviendo Y

                    if EMY > 0.2 and self.PosY <= self.MaxY:
                        self.PosY += self.VelocidadY * EMY* self.TiempoPaso

                    if EMY < -0.2 and self.PosY >= 0:
                        self.PosY += self.VelocidadY * EMY* self.TiempoPaso
    ...
    def ProcesarImagen(self,camara):
        ...
        try:
            ...
            camara, self.PosXDetectado, self.PosYDetectado, self.PosZDetectado, self.FMx, self.FMy, \
    ...
    def PosicionDetectada(self):## en terminos de Cuadricula para la interfase
        return   self.PosXDetectado, self.PosYDetectado, self.PosZDetectado
    def PuntosMapa(self):
    ...
    def PreparaEvaluacion(self, n_evaluaciones=10):
    ...
    def _get_action(self):
        ...
        return   np.array([self.EMX, self.EMY, self.EMA, self.EBE])
    ...
    def _get_obs(self):
        ...
        try:
            ...
            self.ObjetivoAlcanzado = np.array(
                [self.PosXDetectado, self.PosYDetectado, self.PosZDetectado, self.Error])  # el valor Actual de los Objetivos
            ...
            Observaciones = np.divide(self.Observaciones, self.ObservacionesMaximas)
        ...
        while SinErrores == False:
            ...
            try:
                ...
                if self.plantasrandom:
                    ...
                    if self.ContadorPlantas < len(self.Plantas)-1:
    ...
    def paso(self, action):
        ...
        try:
            ...
            if self.ModoVirtual == True:
                ...
                self.EjecutarAccion(EMX=self.EMX, EMY=self.EMY, EBE=self.EBE, EMA=self.EMA, Reset=0)



                diferenciaenms =0

                while diferenciaenms < self.TiempoPaso:

                    self.TiempoActual = datetime.datetime.now()

                    dt = self.TiempoActual - self.TiempoPasoAnterior
                    diferenciaenms = (dt.days * 24 * 60 * 60 + dt.seconds) * 1000 + dt.microseconds / 100
